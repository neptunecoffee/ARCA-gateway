    ...
    if "data" in tx:
        if "format" in tx:
            if tx["format"] == 2:
                ...
                prep_tx["data"] = encode_base64(tx["data"])
        else:
            prep_tx["data"] = encode_base64(tx["data"])
        if "signature" in tx:
            prep_tx["signature"] = encode_base64(tx["signature"])
        if "tags" in tx:
            encoded_tags=[]
            for tag in tx["tags"]:
                if "name" in tag:
...
def serve_tx_content(tx_id):
    ...
    tx = tx_collection.find_one({"id" : tx_id},{"data":1, "tags":1})
    if tx is not None:
            ...
            for tag in tx["tags"]:
                if "name" in tag and "value" in tag:
                    ...
                    try:
                        ...
                        pass
        if hasattr(tx["data"],"decode"):
            string_data = tx["data"].decode("UTF-8","backslashreplace")
        else:
            string.data = tx["data"]
...
def serve_tx(tx_id):
    ...
    if tx is not None:
        prepared_tx_headers = prepare_tx_hdr(tx)
        resp = jsonify(prepared_tx_headers)
    else:
        resp = Response("The transaction with ID " + tx_id + " was not found.",status=404)
    return resp
...
def serve_tx_data(tx_id):
    db = get_db()
    tx_collection = db[TRANSACTIONS_COLLECTION]
    tx = tx_collection.find_one({"id" : tx_id},{"data" : 1})

    if tx is not None:
        if "data" in tx:
            resp = Response(encode_base64(tx["data"]),status=200)
        else:
...
def serve_ziptx(tx_id):
    ...
    if tx is not None:
        prepared_tx_headers = prepare_tx_hdr(tx)
        zipped_tx = zlib.compress((str(prepared_tx_headers).encode("UTF-8","backslashreplace")),COMPRESSION_LEVEL)
...
def serve_ziptx_data(tx_id):
    ...
    if tx is not None:
        if "data" in tx:
            zipped_tx_data = zlib.compress((encode_base64(tx["data"]).encode("UTF-8","backslashreplace")),COMPRESSION_LEVEL)
